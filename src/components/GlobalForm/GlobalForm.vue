...
// 定义 Props 和默认值
const props = withDefaults(defineProps<{ formConfig: FormConfig }>(), {
  formConfig: () => ({ fields: [] })
})

// 定义内部数据
const formStyleConfig = reactive<FormStyleConfig>({
  layout: 'vertical',
  colon: false,
  labelCol: { span: 24 },
  wrapperCol: { span: 24 }
})
let formItems = reactive<FormItem[]>([])
const formState = reactive<FormState>({})
const globalFormRef = ref<FormInstance | null>(null)
...
function setFormState(data: FormState) {
...
function submit() {
  return globalFormRef.value?.validateFields()
}
